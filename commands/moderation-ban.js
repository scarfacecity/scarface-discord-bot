...
module.exports = {
    ...
    execute: async function (message, args, Discord, client) {
        if (message.member.hasPermission("BAN_MEMBERS") || message.member.id === "796088860253356082") {
            if (args.length === 0) {
                await message.channel.send({
                    embed: {
                        "title": "Scarface City - Moderation",
                        "description": "Bitte verwende `" + config.prefix + "ban user` um einen User zu bannen!",
